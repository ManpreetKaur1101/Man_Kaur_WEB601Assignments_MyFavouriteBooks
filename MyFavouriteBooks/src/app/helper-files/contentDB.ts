import { Content } from "./content-interface"

export const contentArray: Content[] = [
    
    { 
        id: 1, 
        title: 'Wrong Place Wrong Time ', 
        description: 'Description for Content 1', 
        creator: 'Creator 1', 
        imgURL: '', 
        type: "", 
        tags: ['Tag1', 'Tag2'] 
      },
      { 
        id: 2, 
        title: 'What Does not kill You', 
        description: 'Description for Content 2', 
        creator: 'Creator 2', 
        imgURL: 'https://www.refinery29.com/images/10229895.jpg', 
        type: '', 
        tags: [] 
      },
      { 
        id: 3, 
        title: 'It Ends With Us', 
        description: 'Description for Content 3', 
        creator: 'Creator 3', 
        imgURL: 'https://product-images-cdn.liketoknow.it/n55_eSfsWH0uGQh0KSVRN7Oorh3IxTG9aepSQjBUC75ZzmGxJNUdLBUdAuGP5KhMZZ2g4UNM5xmQ3qHmUuydfqLp4cOFdGiDf_LfMrLQ3WGRkBbmn7aUPCgoXOu0nR1HcRmXUmQH.1w1Umvt5AN_h9nFllKaxXPTTauY85S2LWfDmWME..ff4opw9Jk-', 
        type: '', 
        tags: ['Tag5', 'Tag6'] 
      },
      { 
        id: 4, 
        title: 'Content 4', 
        description: 'Description for Content 3', 
        creator: 'Creator 3', 
        imgURL: 'https://rathertoofondofbooks.files.wordpress.com/2019/10/39940912._sy475_-1.jpg?w=630', 
        type: 'Thrill', 
        tags: ['Tag5', 'Tag6'] 
      },  { 
        id: 5, 
        title: 'Harry Potter', 
        description: 'Description for Content 3', 
        creator: 'Creator 3', 
        imgURL: 'https://hips.hearstapps.com/hmg-prod/images/harry-potter-and-the-philosopher-s-stone-1615905532.jpg?crop=1xw:1xh;center,top&resize=980:*', 
        type: 'Thrill', 
        tags: ['Tag5', 'Tag6'] 
      },
      { 
        id: 6, 
        title: 'Tonight I Burn', 
        description: 'Description for Content 3', 
        creator: 'Creator 3', 
        imgURL: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRlUqhuif5PpUflqVeF5juTbVDgdpr-kyKOVdlx_uHYmZpFP44VAf2HIOlnmxO_kPG-7Ow&usqp=CAU', 
        type: 'Thrill', 
        tags: ['Tag5', 'Tag6'] ,
      },
      { 
        id: 7, 
        title: 'In THw Heart Of Sea', 
        description: 'Description for Content 7', 
        creator: 'Creator 3', 
        imgURL: 'https://images3.penguinrandomhouse.com/cover/9780141001821', 
        type: 'Romance', 
        tags: ['Tag5', 'Tag6'] 
      },
     {
       id: 8, 
      title: 'The Book Of LOst Names', 
      description: 'Description for Content 7', 
      creator: 'Creator 3', 
      imgURL: 'https://i.gr-assets.com/images/S/compressed.photo.goodreads.com/books/1573742414l/52762903.jpg', 
      type: 'Romance', 
      tags: ['Tag5', 'Tag6'] 
    },
    {
    id: 9, 
    title: 'Stephen King ', 
    description: 'Description for Content 7', 
    creator: 'Creator 3', 
    imgURL: 'https://miblart.com/wp-content/uploads/2023/04/9781529355413-667x1024.jpg', 
    type: 'Romance', 
    tags: ['Tag5', 'Tag6'] 
  }

    

]